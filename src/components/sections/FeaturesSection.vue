<template>
  <section id="features" class="section">
    <div class="container">
      <!-- <h2 class="section-title">Why Choose Our SDK?</h2> -->
      <div class="features">
        <div class="col">
          <article class="card-small fade-in-up">
            <h3 class="card-title">{{ features[0].title }}</h3>
            <p class="card-description">{{ features[0].description }}</p>
          </article>
          <article class="card-large fade-in-up">
            <h3 class="card-title">{{ features[2].title }}</h3>
            <p class="card-description">{{ features[2].description }}</p>
          </article>
        </div>
        <div class="col">
          <article class="card-large fade-in-up">
            <h3 class="card-title">{{ features[1].title }}</h3>
            <p class="card-description">{{ features[1].description }}</p>
          </article>
          <article class="card-small fade-in-up">
            <h3 class="card-title">{{ features[3].title }}</h3>
            <p class="card-description">{{ features[3].description }}</p>
          </article>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">
const features = [
  {
    id: 1,
    title: 'Advanced yield',
    description:
      'Give users access to curated DeFi strategies with competitive APY and automatic rebalancing logic behind a single call.',
  },
  {
    id: 2,
    title: 'No Web3 expertise required',
    description:
      'We handle wallets, signing, gas, and protocol routing under the hood. You ship a clean, familiar UX for your users and a simple API for your team.',
  },
  {
    id: 3,
    title: 'Open-source core',
    description:
      'MIT-licensed SDK you can audit and extend. Add premium features (analytics, routing policies, priority support) via subscription when you need them.',
  },
  {
    id: 4,
    title: 'Quick integration',
    description:
      'Use the Server SDK or React SDK and go live fast. No wallet plumbing, no protocol juggling, no custom bridges.',
  },
];
</script>

<style scoped>
.features {
  width: min(92vw, 300px);
  aspect-ratio: 1 / 1;
  display: flex;
  gap: 24px;
  padding: 24px;
  box-sizing: border-box;
  margin: 0 auto;
  justify-content: center;
  align-items: center;
}

.col {
  display: flex;
  flex-direction: column;
  gap: 24px;
}

.card-small {
  width: 226px;
  height: 137px;
  background: rgba(255, 255, 255, 0.08);
  border-radius: 20px;
  padding: 20px;
  box-shadow: 0 6px 24px rgb(0 0 0 / 0.25) inset;
  display: flex;
  flex-direction: column;
  justify-content: center;
}

.card-large {
  width: 226px;
  height: 172px;
  background: rgba(255, 255, 255, 0.08);
  border-radius: 20px;
  padding: 20px;
  box-shadow: 0 6px 24px rgb(0 0 0 / 0.25) inset;
  display: flex;
  flex-direction: column;
  justify-content: center;
}

.card-title {
  font-size: 14px;
  margin-bottom: 0.5rem;
}

.card-description {
  font-size: 12px;
}

@media (max-width: 520px) {
  .features {
    display: flex;
    flex-direction: column;
  }
}
</style>
